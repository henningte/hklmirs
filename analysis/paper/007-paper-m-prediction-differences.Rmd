  ...
  dplyr::bind_rows(
    ...
    d %>%
      dplyr::mutate(
        hol_hat =
          m1.10 %>%
          posterior_predict(newdata = d_underfitting[[4]]) %>%
          as.data.frame() %>%
          purrr::map_dbl(median),
         kl_hat = 
          m2.7 %>%
          posterior_predict(newdata = d_underfitting[[1]]) %>%
          as.data.frame() %>%
          purrr::map_dbl(median),
    ...
    d_peat %>%
      dplyr::mutate(
        hol_hat =
          m1.10 %>%
          posterior_predict(newdata = d_peat_bin50) %>%
          as.data.frame() %>%
          purrr::map_dbl(median),
         kl_hat = 
          m2.7 %>%
          posterior_predict(newdata = d_peat_bin10) %>%
          as.data.frame() %>%
          purrr::map_dbl(median),
        model = "Best binned spectra"
      ) %>%
    ...
    d_veg %>%
      dplyr::mutate(
        hol_hat =
          m1.10 %>%
          posterior_predict(newdata = d_veg_bin50) %>%
          as.data.frame() %>%
          purrr::map_dbl(median),
         kl_hat = 
          m2.7 %>%
          posterior_predict(newdata = d_veg_bin10) %>%
          as.data.frame() %>%
          purrr::map_dbl(median),
        model = "Best binned spectra"
      ) %>%
...
d_pred_diff_intensities <-
  d_pred_diff_spectra %>%
  ir::ir_get_intensity(wavenumber = c(3400, 1200, 1550)) %>%
  ir::ir_drop_spectra() %>%
...
p_pred_diff_train1_p1 <-
  ...
  facet_wrap( ~ model) +
  guides(size = guide_legend(title = expression(Absorbance~at~1550~cm^{-1})),
...
# in the plots above, one can see that the model makes different predictions for two OM classes: needles and leaves and grasses. How do these spectra differ? It seem that this is mainly th region around 1550
d %>% 
...
p_pred_diff_hol_intensity2 <-
  ...
  dplyr::filter(p_index) %>%
  dplyr::mutate(data_type = factor(data_type, levels = data_type_levels %>% rev())) %>%
  ggplot(aes(x = intensity_1200 %>% scale(), y = hol_hat_or - hol_hat, colour = data_type)) +
  geom_hline(yintercept = 0, colour = "grey50") +
  geom_point(alpha = 0.3, shape = 16) + 
  geom_smooth(formula = y ~ x, method = "loess", se = FALSE) +
  facet_grid(data_type ~ cut_number(carb, n = facet_n_level)) +
  labs(y = "Prediction difference", x = expression("Intensity at 1200 cm"^{-1}), 
       title = expression("Intensity at 1200 cm"^{-1})) +
  guides(colour = guide_legend(title = "Dataset")) +
  theme(legend.position = "bottom") +
  scale_color_manual(
    breaks = data_type_levels, 
...
p_pred_diff_hol_intensity3 <-
  ...
  dplyr::filter(p_index) %>%
  dplyr::mutate(data_type = factor(data_type, levels = data_type_levels %>% rev())) %>%
  ggplot(aes(x = intensity_1550 %>% scale(), y = hol_hat_or - hol_hat, colour = data_type)) +
  geom_hline(yintercept = 0, colour = "grey50") +
  geom_point(alpha = 0.3, shape = 16) + 
  geom_smooth(formula = y ~ x, method = "loess", se = FALSE) +
  facet_grid(data_type ~ cut_number(carb, n = facet_n_level)) +
  labs(y = "Prediction difference", x = expression("Intensity at 1550 cm"^{-1}), 
       title = expression("Intensity at 1550 cm"^{-1})) +
  guides(colour = guide_legend(title = "Dataset")) +
  theme(legend.position = "bottom") +
  scale_color_manual(
    breaks = data_type_levels, 
...
d_pred_diff_intensities %>%
  ...
  dplyr::filter(p_index_test) %>%
  ggplot(aes(x = intensity_1200 %>% scale(), y = hol_residuals, colour = model)) + 
  geom_hline(yintercept = 0, colour = "grey50") +
  geom_point() +
  geom_smooth(formula = y ~ x, method = "loess", se = FALSE) +
  facet_grid(model ~ cut(carb, breaks = c(-1.45, -0.883, 0.454, 1.16))) +
...
d_pred_diff_intensities %>%
  ...
  dplyr::filter(p_index_test) %>%
  ggplot(aes(x = intensity_1550 %>% scale(), y = hol_residuals, colour = model)) +
  geom_hline(yintercept = 0, colour = "grey50") +
  geom_point() +
  geom_smooth(formula = y ~ x, method = "loess", se = FALSE) +
  facet_grid(model ~ cut(carb, breaks = c(-1.45, -0.883, 0.454, 1.16))) +
...
p_pred_diff_kl_histogram1 <-
  ...
  facet_wrap(~ cut_number(arom15arom16_rel, n = 4)) +
