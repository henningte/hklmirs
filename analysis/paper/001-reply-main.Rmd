...
1. The models described in @Hodgkins.2018 to predict holocellulose and Klason lignin contents from mid infrared spectra, but recomputed in a Bayesian framework. This is the "original Bayesian model" in [---todo:ref]. These models were used to reproduce the computations in @Hodgkins.2018. Here, these models are called "original" models.

2. A modified model to predict each, holocellulose and Klason lignin contents from mid infrared spectra. These are the models called "best binned spectra" models in [---todo:ref]. A brief description of these models is given in section [Computation of modified models to predict organic matter holocellulose and Klason lignin contents from mid infrared spectra]. These models are used to reanalyze holocellulose and Klason lignin depth profiles for the two latitude classes defined in @Hodgkins.2018, as described in section [Depth profiles].
...
1. Since representative training data are missing, both the original and modified models cannot be fully validated and prediction uncertainties are probably larger than computed with these models [---todo:ref]. Since representative test data are missing, these additional uncertainties cannot be estimated. We assume that these uncertainties are larger than those induced by parameter uncertainty from models for the depth profiles. 
...
The modified models using only the MIRS were also used to compute median values and confidence intervals mentioned in the main text in section "Klason lignin content and residual enrichment in (sub)tropical peatlands probably are larger". In these computations, we also considered the predictive uncertainty of the original models (the Bayesian formulation described in ---todo:ref).
...
\phi &\sim \text{beta}(`r s1_data_holocellulose$phi_a`, `r s1_data_holocellulose$phi_b/s1_data_holocellulose$phi_scale`)\\
...
Where $\text{Y}_1$ are the measured holocellulose contents of the training data; $\mu_{1}$ are the respective mean values modeled with the MIRS, only; $i$ is an index for each sample of the training data; $Y_{2}$ are the estimated peat holocellulose contents modeled simultaneously from the MIRS ($\mu_{2}$) and from the average average near-surface peat holocellulose contents per core ($\mu_{\text{core}}$); $j$ is an index for each sample in the peat data; $\text{index\_core}$ is a vector assigning each sample in the peat data to the respective core index; $k$ is an index for each unique core in the peat data. The average average near-surface peat holocellulose contents per core ($\mu_{\text{core}}$) are in turn modeled in dependency on the latitude.

The full model for Klason lignin is:
...
\alpha &\sim \text{normal(0, 0.2)}\\
\beta &\sim \text{horseshoe}(zb, hs_{local}, hs_{global}, hs_{scale\_slab}^2 \cdot hs_{slab})\\
\phi &\sim \text{beta}(`r s1_data_klason_lignin$phi_a`, `r s1_data_klason_lignin$phi_b/s1_data_klason_lignin$phi_scale`)\\
Y_{2}[j] &\sim \text{beta}(\mu_{\text{core}}[\text{index\_core}[j]] \phi_{\text{core}}[\text{index\_core}[j]], (1-\mu_{\text{core}}[\text{index\_core}[j]])\phi_{\text{core}}[\text{index\_core}[j]])\\
\mu_{\text{core}}[k] &\sim \text{beta}(\mu_{\text{pop}}[k] \phi_{\text{pop}}, (1-\mu_{\text{pop}}[k])\phi_{\text{pop}})\\
\mu_{\text{pop}}[k] &= \text{logit}^{-1}\left(\alpha_{\text{latitude}} + \beta_{\text{latitude}}\text{x}_{\text{latitude}}[k]\right)\\
...
#                   lwr = quantile(y_hat, probs = 0.05),
#                   upr = quantile(y_hat, probs = 0.95),
#                   sampling_latitude = mean(sampling_latitude),
#                   .groups = "drop") %>%
#  dplyr::mutate(
#    sampling_latitude = dplyr::case_when(model != "Original" ~ sampling_latitude + 3,
#                                         TRUE ~ sampling_latitude)
#    )

#d_peat_pred_best_summary1 <-
#  d_peat_pred_best %>%
#  dplyr::group_by(variable_y, model, core_label, sample_id) %>%
#  dplyr::summarise(fit = mean(y_hat),
#                   lwr = quantile(y_hat, probs = 0.05),
#                   upr = quantile(y_hat, probs = 0.95),
#                   sample_depth_lower = mean(sample_depth_lower),
#                   sampling_latitude = mean(sampling_latitude),
#                   .groups = "drop")

## compute average and prediction intervals for latitude surface peat contents linear model
#x_seq <- seq(min(d_peat_pred_best$sampling_latitude, na.rm = TRUE), max(d_peat_pred_best$sampling_latitude, na.rm = TRUE), by = 1)
#d_peat_pred_best_latitude <-
#  d_peat_pred_best %>%
#  dplyr::filter(iter <= 1000 & sample_depth_lower <= 50) %>%
#  dplyr::group_by(variable_y, model, iter, core_label) %>%
#  dplyr::summarise(y_hat = mean(y_hat),
#                   sampling_latitude = mean(sampling_latitude),
#                   .groups = "drop_last") %>%
#  dplyr::summarise(
#    lm_fit = {
#      m <- lm(y_hat ~ sampling_latitude, 
#                 data=tibble::tibble(y_hat = y_hat, sampling_latitude = sampling_latitude))
#      res <- predict(m, newdata = data.frame(sampling_latitude = x_seq), interval = "confidence", level = 0.95)
#      tibble::tibble(y_hat = res[, 1],
#                     lwr = res[, 2],
#                     upr = res[, 3],
#                     sampling_latitude = x_seq) %>% list()
#      }
#  ) %>%
#  tidyr::unnest(cols = dplyr::contains("lm_fit")) %>%
#  dplyr::group_by(variable_y, model, sampling_latitude) %>%
#  dplyr::summarise(fit = mean(y_hat),
#                   lwr = quantile(lwr, 0.025, na.rm = TRUE),
#                   upr = quantile(upr, 0.975, na.rm = TRUE),
#                   .groups = "drop")

#d_peat_pred_best_latitude_slopes <-
#  d_peat_pred_best %>%
#  dplyr::filter(iter <= 1000 & sample_depth_lower <= 50) %>%
#  dplyr::group_by(variable_y, model, iter, core_label) %>%
#  dplyr::summarise(y_hat = mean(y_hat),
...
#                 data=tibble::tibble(y_hat = y_hat, sampling_latitude = sampling_latitude))
#      m <- summary(m)
#      tibble::tibble(intercept = coef(m)[1, 1],
#                     slope = coef(m)[2, 1],
#                     slope_p = coef(m)[2, 4]) %>% list()
#      }
#  ) %>%
#  tidyr::unnest(cols = dplyr::contains("lm_fit")) %>%
#  dplyr::group_by(variable_y, model) %>%
#  dplyr::summarise(intercept_mean = mean(intercept),
#                   slope_mean = mean(slope),
#                   slope_lwr = quantile(slope, 0.025, na.rm = TRUE),
#                   slope_upr = quantile(slope, 0.975, na.rm = TRUE),
#                   slope_p_mean = mean(slope_p),
#                   slope_p_lwr = quantile(slope_p, 0.025, na.rm = TRUE),
#                   slope_p_upr = quantile(slope_p, 0.975, na.rm = TRUE),
#                   .groups = "drop")
...
```{r m-reply-predictions-veg, include=FALSE, eval=FALSE}
## vegetation data
...
d_veg_pred_best <- 
  dplyr::bind_rows(
    m1.8 %>%
      ...
      tidyr::pivot_longer(cols = dplyr::everything(), 
                          ...
                          values_to = "y_hat") %>%
      dplyr::mutate(
        iter = seq_len(length(y_hat)/nrow(d_veg)) %>% rep(each = nrow(d_veg)),
        sample_id = d_veg$sample_id %>% rep(length(unique(iter))),
        model = "Best binned spectra",
        variable_y = "Holocellulose"
      ),
    m1.2 %>%
  ...
  dplyr::left_join(d_veg %>% dplyr::select(-spectra), by = "sample_id") %>%
  dplyr::filter(iter <= 2000)
...
# compute average values per site
d_peat_pred_best_turnover_summary <-
  d_peat_pred_best %>%
  dplyr::filter(sample_depth_lower <= 50) %>%
  dplyr::group_by(variable_y, model, barchart_label, iter) %>%
  dplyr::summarise(y_hat = mean(y_hat),
                   .groups = "drop") %>%
  dplyr::arrange(barchart_label, iter)

d_veg_pred_best_turnover_summary <-
  d_veg_pred_best %>%
  dplyr::group_by(variable_y, model, barchart_label, iter) %>%
  dplyr::summarise(y_hat = mean(y_hat),
                   .groups = "drop") %>%
...
d_turnover <-
  ...
  dplyr::mutate(
    Peat = y_hat,
    Vegetation = d_veg_pred_best_turnover_summary$y_hat,
    Difference = Peat - Vegetation
  ) %>%
  ...
  tidyr::pivot_longer(cols = dplyr::one_of(c("Peat", "Vegetation", "Difference")),
                      names_to = "sample_type",
                      values_to = "values") %>%
  dplyr::group_by(variable_y, model, barchart_label, sample_type) %>%
  dplyr::summarise(.groups = "drop",
                   fit = median(values) * 100,
                   lwr = quantile(values, prob = 0.025) * 100,
                   upr = quantile(values, prob = 0.975) * 100)
```


```{r m-reply-p-sites-latitude, include=FALSE, eval = FALSE}
...
p1 <-
  ...
  ggplot(aes(y = fit * 100, ymin = lwr * 100, ymax = upr * 100,
             x = sampling_latitude, 
             colour = model, 
             fill = model)) +
  geom_path() +
  ...
  geom_errorbar(data = 
                  ...
                  dplyr::filter(variable_y == "Holocellulose")  %>%
                  dplyr::mutate(model = ifelse(model == "Best binned spectra", "Modified", model)) %>%
                  dplyr::mutate(sampling_latitude = ifelse(model == "Original", sampling_latitude, sampling_latitude + 2)),
                aes(y = fit * 100, ymin = lwr * 100, ymax = upr * 100, 
                    x = sampling_latitude, 
                    colour = model)) +
  geom_point(data = 
               d_peat_pred_best_summary %>%
               ...
               dplyr::mutate(model = ifelse(model == "Best binned spectra", "Modified", model)) %>%
               dplyr::mutate(sampling_latitude = ifelse(model == "Original", sampling_latitude, sampling_latitude + 2)),
             aes(y = fit * 100, x = sampling_latitude, colour = model)) +
...
```{r m-reply-p-veg-peat-difference, eval=FALSE, include=FALSE}
## plots of difference between vegetation and surface peat contents
p1 <-
  d_turnover %>%
  dplyr::filter(variable_y == "Holocellulose") %>%
  dplyr::mutate(model = ifelse(model == "Best binned spectra", "Modified", model)) %>%
  ...
  geom_hline(yintercept = c(-0.2, 0, 0.2, 0.4) * 100, colour = "grey", linetype = 2) +
...
p2 <-
  ...
  coord_flip() +
  labs(title = "Klason lignin", 
       y = expression(Klason~lignin~"[mass-%]"),
       x = "Site category") +
  facet_wrap(~ model) +
  guides(fill = guide_legend(title = "")) +
  scale_fill_grey()
