...
abstract:  |
    To understand global soil organic matter (SOM) chemistry and its dynamics, we need tools to efficiently quantify SOM properties, for example prediction models using mid infrared spectra. The advantages of such models rely on their validity and accuracy. Recently, \cite{Hodgkins.2018} developed models to predict organic matter holocellulose and Klason lignin contents, two indicators of SOM stability and major fractions of organic matter in general. The models can be used to understand large-scale SOM gradients and have been used in various studies.  
...
Overall, this indicates that the prediction domain formed by the training data does not cover peat --- particularly decomposed peat --- and partly peat forming vegetation samples. This is probably also true for non-peat SOM. Therefore the models' predictions can represent extrapolations in practice; the training data is not in general representative for peat and peat-forming vegetation.
