...
# ---todo: brms and rstanarm yield very different uncertainty estimates. brms has smaller prediction intervals. The reason seems to be the different priors for phi (normal(0, 2.5) for rstanarm; gamma(0.01, 0.01) for brms).
```
...
p1 <-
  ...
  geom_path() +
  geom_point(data = d_peat_pred %>% dplyr::filter(variable_y == "Holocellulose" & model != "Gaussian, non-Bayesian"), 
             mapping = aes(x = fit_gaussian, y = fit)) +
  geom_point(data = d_veg_pred %>% dplyr::filter(variable_y == "Holocellulose" & model != "Gaussian, non-Bayesian"), 
...
p2 <-
  ...
  geom_ribbon(aes(ymin = lwr, ymax = upr), alpha = 0.2, colour = NA) +
  geom_path() +
  geom_point(data = d_peat_pred %>% dplyr::filter(variable_y == "Klason lignin" & model != "Gaussian, non-Bayesian"), 
             mapping = aes(x = fit_gaussian, y = fit)) +
  geom_point(data = d_veg_pred %>% dplyr::filter(variable_y == "Klason lignin" & model != "Gaussian, non-Bayesian"), 
             mapping = aes(x = fit_gaussian, y = fit), shape = 4) +
